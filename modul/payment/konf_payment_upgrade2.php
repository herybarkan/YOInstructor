...
?>
<?php
 $select_pay=$db->prepare("SELECT * FROM payments_upgrade WHERE product_id='$_GET[pid]'"); //sql select query
   $select_pay->execute();
...
$uptgl=$db->prepare("UPDATE payments_upgrade SET tgl_start='$data[2]', tgl_end='$data[3]', tgl_remind='$data[4]' WHERE product_id='$_GET[pid]'");
$uptgl->execute();
...
$mailz->setFrom('system@yoinstructor.com', 'Upgrade Account Payment');
$mailz->addReplyTo('system@yoinstructor.com', 'Upgrade Account Payment');
...
// Subjek email
$mailz->Subject = 'Upgrade Account Payment';
...
$mailz->isHTML(true);
  ...
  <tr>
    ...
    <span>
    <h2>My Upgrade Payment Info</h2></span>
    </td>
  </tr>
  <tr>
    <td width="34%">&nbsp;</td>
  ...
  <tr>
    ...
    <td>waiting for checking</td>
  </tr>
  <tr>
    <td>Account Name Owner</td>
    <td>'.$row['nm_account'].'</td>
  </tr>
  <tr>
    <td>Bank</td>
    <td>'.$row['bank'].'</td>
...
$mailz2->setFrom('system@yoinstructor.com', 'My Upgrade Account Payment');
$mailz2->addReplyTo('system@yoinstructor.com', 'My Upgrade Account Payment');
...
// Subjek email
$mailz2->Subject = 'My Upgrade Account Payment';
...
<div class="edgtf-full-width">
   <div class="edgtf-full-width-inner posisi">
      ...
      <p>Payment Number: <?php echo $row['product_id']; ?></p>
      <p>Account Name Owner: <?php echo $row['account_name']; ?></p>
      <p>Bank: <?php echo $row['bank']; ?></p>
      <p>Account Number: <?php echo $row['no_rek']; ?></p>
      <p>Payment Status: Waiting for Checking</p>
      <p>Amount: <?php echo $row['currency_code']; ?> <?php echo $row['payment_amount']; ?></p>
      <p>Date Transfer: <?php echo $row['tgl_trf']; ?></p>
      
      
      <p>Payment Name: Upgrade to Profesional Account</p>
